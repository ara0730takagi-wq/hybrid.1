    ...
    def calculate_edr(param_dict, arcmethod):
        ...
        for j in tqdm(range(N)):
            for i in range(n_attempts):#1s=100μs(t_AFC)*10000,論文では取りあえず10000回
                tl,ARC_success = trial_timeline1(p_EL=p_arc_gen, p_EC=eta_conn, p_QST=eta_QR, rng=rng)
                if ARC_success==True:
                    successes +=1      
        p=successes/N
        ...
        for j in tqdm(range(N)):
            for i in range(n_attempts):#1s=100μs(t_AFC)*10000,論文では取りあえず10000回
                tl,ARC_success = trial_timeline2(p_EL=p_arc_gen, p_EC=eta_conn, p_QST=eta_QR, rng=rng)
                if ARC_success==True:
                    successes +=1      
        p=successes/N
        error=np.sqrt(p*(1.0-p)/N) 
