...
# ------------------- 使い方例 -------------------
# p_EL = p_arc_gen  (MethodBで l->l/2 にした後に計算した値)
# p_EC = eta_conn
# p_QST = eta_QR
# omega_epps = eta_EPPS * R_EPPS
# t_gen = ttrans（= t_QST + t_ARC + t_CNOT）  ※あなたの calc_timings で言う ttrans

# tau_star, info = find_tau_for_target_prob(
#     target_p=0.95,
#     omega_epps=omega_epps,
#     t_gen=ttrans,
#     p_EL=p_arc_gen,
#     p_EC=eta_conn,
#     p_QST=eta_QR,
#     num_big_trials=5000,
#     seed=0,
#     tau_max=None,   # QR保持時間で上限あるなら t_QR を入れる
# )
# print("tau_star =", tau_star)
# print("p,se,M  =", info)
# 2. 図形描画ヘルパー
# ============================
def draw_QR(ax, x, y, color):
    r = 0.25
    circle = patches.Circle((x, y), radius=r, facecolor=color, edgecolor="black")
    ax.add_patch(circle)

def draw_QST(ax, x, y, left_color, right_color):
    """QST：縦長長方形（左/右で別色）"""
    w, h = 0.6, 0.8
    # 左半分
    rect_l = patches.Rectangle((x - w/2, y - h/2), w/2, h,
                               facecolor=left_color, edgecolor="black")
    # 右半分
    rect_r = patches.Rectangle((x, y - h/2), w/2, h,
                               facecolor=right_color, edgecolor="black")
    ax.add_patch(rect_l)
    ax.add_patch(rect_r)

def draw_EL(ax, x, y, color):
    """EL：横長長方形（成功:青 / 失敗:茶色）"""
    w, h = 0.8, 0.3
...
def draw_EC(ax, x, y, left_color, right_color, mode="partial"):
    """
    EC：三角形
    mode="partial" : 左右半分を left_color/right_color で塗り分け
    mode="full"    : 全体を left_color で塗る（判定後 Yellow / 茶色）
    """
    w, h = 0.8, 0.6
    top = (x, y + h/2)
    ...
    if mode == "partial":
        # 左半分
        verts_l = [top, mid_bottom, left_bottom]
        tri_l = patches.Polygon(verts_l, closed=True,
                                facecolor=left_color, edgecolor="black")
        # 右半分
        verts_r = [top, right_bottom, mid_bottom]
        tri_r = patches.Polygon(verts_r, closed=True,
                                facecolor=right_color, edgecolor="black")
        ax.add_patch(tri_l)
        ax.add_patch(tri_r)
    else:
        verts = [top, left_bottom, right_bottom]
        tri = patches.Polygon(verts, closed=True,
                              facecolor=left_color, edgecolor="black")
        ax.add_patch(tri)

def draw_frame(tl, time_key, frame_index, title_text):
    """
    tl        : trial_methodB_timeline が返したタイムライン dict
    time_key  : "t0", "t_quarter", "t_half", "t_full"
    frame_index : [1枚目], [2枚目], ...
    """
    ...
    # 位置とラベル
    x_positions = [0, 1.2, 2.4, 3.6, 4.8, 6.0, 7.2]
    names = ["QR1_1", "QST1_1", "EL1_1", "EC1_1", "EL1_2", "QST1_2", "QR1_2"]
    y_center = 1.3
    y_label = 0.4
    ...
    EC_left = "lightgray"
    EC_right = "lightgray"
    EC_mode = "partial"  # "partial" or "full"
    QST1_left = "lightgray"
    ...
    if time_key == "t0":
        ...
        EL1_color = "blue" if EL1 else "saddlebrown"
        EL2_color = "blue" if EL2 else "saddlebrown"

    elif time_key == "t_half":
        ...
        # EC 判定のルール
        if not EC_judged:
            # 判定せず：左右にヘラルドを反映（成功→green, 失敗→茶色）
            EC_left = "blue" if msg1 else "saddlebrown"
            EC_right = "blue" if msg2 else "saddlebrown"
            EC_mode = "partial"
        else:
            # 両ヘラルド成功で判定実施
            if EC_success:
                # 成功 → EC 全体 Yellow、EL はそのまま
                EC_left = "yellow"
                EC_mode = "full"
            else:
                # 失敗 → EC 全体茶色 + 両 EL を茶色に
                EC_left = "saddlebrown"
                EC_mode = "full"
                EL1_color = "saddlebrown"
                EL2_color = "saddlebrown"

        # QST 左半分：t_AFC/2 で EL から来たヘラルド
        q1 = tl["t_half"]["QST1_msg_from_EL1"]
        q2 = tl["t_half"]["QST2_msg_from_EL2"]
        QST1_left = "blue" if q1 else "saddlebrown"
        QST2_right = "blue" if q2 else "saddlebrown"
        # 右半分はまだヘラルドが届いていないので gray のまま

    elif time_key == "t_full":
        # ベースとして EL の色をセット
        ...
        # QST に届いているヘラルド（左右）
        q1_L = tl["t_full"]["QST1_msg_from_EL1"]  # EL1_1 → QST1_1
        q1_R = tl["t_full"]["QST1_msg_from_EL2"]  # EL1_2 → QST1_1
        q2_L = tl["t_full"]["QST2_msg_from_EL2"]  # EL1_2 → QST1_2
        q2_R = tl["t_full"]["QST2_msg_from_EL1"]  # EL1_1 → QST1_2

        # 基本は「ヘラルドに従った色」
        QST1_left = "blue" if q1_L else "saddlebrown"
        QST1_right = "blue" if q1_R else "saddlebrown"
        QST2_right = "blue" if q2_L else "saddlebrown"
        QST2_left = "blue" if q2_R else "saddlebrown"
        ...
        if QST_judged:
            if ARC_success:
                ...
                EC_left = "saddlebrown"
                EC_mode = "full"
                QST1_left = QST1_right = "saddlebrown"
                QST2_left = QST2_right = "saddlebrown"
        else:
            # 判定をしていない場合は、ヘラルドに応じた色のまま維持
            pass

    # -------- 実際に描画 --------
    for x, name in zip(x_positions, names):
        if name == "QR1_1":
            draw_QR(ax, x, y_center, QR1_color)
        elif name == "QR1_2":
            draw_QR(ax, x, y_center, QR2_color)
        elif name == "QST1_1":
            draw_QST(ax, x, y_center, QST1_left, QST1_right)
        elif name == "QST1_2":
            draw_QST(ax, x, y_center, QST2_left, QST2_right)
        elif name == "EL1_1":
            draw_EL(ax, x, y_center, EL1_color)
        elif name == "EL1_2":
            draw_EL(ax, x, y_center, EL2_color)
        elif name == "EC1_1":
            draw_EC(ax, x, y_center, EC_left, EC_right, mode=EC_mode)

        ax.text(x, y_label, name, ha="center", va="center", fontsize=9)

    ax.set_title(f"[{frame_index}p] {title_text}")
    plt.tight_layout()
    return fig, ax
