...
eta_dec=0.9
Nfreq=50
eta_afc(AFC memory efficiency)=0.3
t_AFC(AFC strage time)=100μs
EL length=20 km
eta_EPPS=0.01
R_EPPS=10^6 Hz
alpha(optic fiber coefficient)=0.2(eta_fiberに対応)
eta_loss(generic QST segment fiber transmission efficiency)=0.9
"""
@dataclass
class HybridParams:
    # topology
...
def toy_p_click_freq_single_mode(distance_km: float,
    ...
    """
    fiber_trans = np.exp(-eta_fiber * distance_km)
